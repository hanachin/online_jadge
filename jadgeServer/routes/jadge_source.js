...
var compareSource, compileSource, executeError, executeSource, getQueueContents, getTimestamp, inputExecute, kondoMethod, makeDir, noinputExecute, pulloutDecimal, removeDir, removeQueue, replaceEqualsign, replaceFullsizeChar, replaceSource, saveResult, sendMsg, skipTable, toUpper, writeSource, writeTestcase;
...
noinputExecute = function(req, questionNo, path, exec, callBack) {
  var exePath;
  ...
  }, req.argStdout[num] = stdout, callBack(null, 7));
};
...
pulloutDecimal = function(string) {
  var decimal1, decimal2, i, len, number, _ref, _ref1;
  i = 0;
  len = string.length;
  while (i < len) {
    number = string[i];
